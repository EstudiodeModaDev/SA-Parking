...
import type { Collaborator } from '../../Models/colaboradores';
import type { SlotUI } from '../../Models/Celdas';

// ⬇️ Hook para listar colaboradores (versión que recibe el servicio)
...
const ColaboradoresInscritos: React.FC = () => {
  // ====== servicios Graph
  const {  colaboradoresFijos: colaboradoresSvc } = useGraphServices();
  ...
  // (si luego reactivas selección de celdas en el modal)
  const [freeSlots] = React.useState<SlotUI[]>([]);
  ...
  const viewRows = React.useMemo(
    () => vehicleFilter === 'all'
      ? rows
      : rows.filter(r => r.tipoVehiculo === vehicleFilter),
    [rows, vehicleFilter]
  );
  ...
  return (
    <section className={styles.section}>
      <div className={styles.card}>
                    ...
                    {viewRows.map((c) => (
                      <tr key={c.id}>
                        ...
                        <td className={styles.td}>
                          {/* Ver detalles (si lo usas) */}
                          {/* <button
                            type="button"
                            className={styles.iconBtn}
                            onClick={() => { setSelected(c); setIsOpenDetails(true); }}
                            title="Ver detalles"
                          >
                            👁️
                          </button> */}
        ...
        {/* MODAL: Ver detalles (si lo estás usando) */}
        <ModalVerColaborador
